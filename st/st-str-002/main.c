...
#define DMA_DEST2_ADDRESS   0x80013000
#define DMA_DEST_MMIO_ADDRESS   0x80014000
...
volatile uint32_t *dma_dest2_ptr = (volatile uint32_t*)DMA_DEST2_ADDRESS;
volatile uint32_t *dma_dest_mmio = (volatile uint32_t*)DMA_DEST_MMIO_ADDRESS;
...
void dma_test_callback(int error_code, uint64_t user_data){
    ...
    if(xferid != 0) {
        if(xferid % 2){
            ...
            // DEV->MEM
            dma_transfer(DMA_DEV_TO_MEM, (uintptr_t)dma_mmio_ptr, (uintptr_t)dma_dest_mmio, MMIO_TRANSFER_SIZE, DWAXIDMAC_AX_CACHE_DEVICE, DWAXIDMAC_AX_CACHE_CACHEABLE, dma_test_callback, xferid++);
        }
        else{
            for(int i = 0; i < MMIO_TRANSFER_SIZE / TRANSFER_ITEM_SIZE; i++){
...
int main() {
    ...
    if (hartid == 0){
        ...
        // MEM->MEM(NONCACHE)
        dma_transfer(DMA_MEM_TO_MEM, (uintptr_t)dma_src0_ptr, (uintptr_t)dma_dest1_ptr, TRANSFER_SIZE, DWAXIDMAC_AX_CACHE_CACHEABLE, DWAXIDMAC_AX_CACHE_NONCACHE, dma_test_callback, 0);
        // MEM->MEM(NONCACHE)
        dma_transfer(DMA_MEM_TO_MEM, (uintptr_t)dma_src0_ptr, (uintptr_t)dma_dest2_ptr, TRANSFER_SIZE, DWAXIDMAC_AX_CACHE_CACHEABLE, DWAXIDMAC_AX_CACHE_NONCACHE, dma_test_callback, 0);
        // MEM->DEV
    ...
    for(int i = 0; i < TRANSFER_SIZE / TRANSFER_ITEM_SIZE; i++){
        if(*(dma_dest0_ptr + i) != VALUE_INITIAL)
            atomic_printf("CACHEABLE Check Failed!, index %d, val %d\n", i, *(dma_dest0_ptr + i));
    }

    for(int i = 0; i < TRANSFER_SIZE / TRANSFER_ITEM_SIZE; i++){
        if(*(dma_dest1_ptr + i) != VALUE_INITIAL)
            atomic_printf("NONCACHE Check Failed!, index %d, val %d\n", i, *(dma_dest1_ptr + i));
    }

    for(int i = 0; i < TRANSFER_SIZE / TRANSFER_ITEM_SIZE; i++){
        if(*(dma_dest2_ptr + i) != VALUE_INITIAL)
            atomic_printf("NONCACHE Check Failed!, index %d, val %d\n", i, *(dma_dest2_ptr + i));
    }
