...
volatile uint64_t __attribute__((aligned(64))) RES_M[M][M];
...
int main() {
  ...
  uint8_t err;
  atomic_printf("[INFO]: core %lu boot\n", iam);
  ...
  if(0 == iam) atomic_printf("[INFO]: start simm test!\n");
  matrix_multiply_uint16_square(MAT_A_BLOCK(iam), &MAT_B[0][0], RES_BLOCK(iam), BLOCK_ROWS, N, M);
  atomic_printf("[INFO]: core %lu finished job!\n", iam);
  if(barrier(NUM_CORES)) return 1;
  while(iam > 0) riscv_wfi();

  if(0 == iam) atomic_printf("[INFO]: start comparation!\n");
