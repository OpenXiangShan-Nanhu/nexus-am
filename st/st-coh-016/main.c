...
void cbo_inval_region(void *start, size_t size){
    uint8_t *current_addr = (uint8_t *)start;
    uint8_t *end_addr = current_addr + size;

    while(current_addr < end_addr){
        riscv_cbo_inval((uintptr_t)current_addr);
...
bool reader_check(int hartid){
    ...
    for(int k = 0; k < transfer_size; k++){
        if(src_addr[k] != dest_addr[k]){
            atomic_printf("Core %d FAILED!!! Index:%d, src data:%x, dest data:%x\n", hartid, k, src_addr[k], dest_addr[k]);
            return true;
        }
    }
...
int main() {
    ...
    for(int i = 0; i < LITERATION; i++){
        if(hartid == 0){
            ...
            cbo_inval_region((void*)region_c_addr, transfer_size);
            ...
            cbo_inval_region((void*)region_d_addr, transfer_size);
