...
static struct dma_lli g_lli_pool[TOTAL_LLI_POOL_SIZE];
...
static void dma_chan_disable(struct dma_chan *chan){
    ...
    WRITE_U32(dmac->base_addr + DMAC_CFG, (uint32_t)val);
}
...
static void dma_controller_init(){
    ...
    for(int i = 0; i < NUM_DMA_CONTROLLERS; i++){
        ...
        for(int j = 0; j < DMAC_MAX_CHANNELS; j++){
            ...
            chan->base_addr = g_dma_controllers->base_addr + j * COMMON_REG_LEN;
        }
        dma_chan_init(dmac);
        dma_irq_enable(dmac);
        dma_enable(dmac);
...
static int dma_prepare_llp(struct dma_desc *desc, uint64_t src_addr, uint64_t dst_addr, uint32_t total_length){
    ...
    for(uint32_t i = 0; i < num_blocks; i++) {
        ...
        current_lli->block_ts_lo = block_size;
        current_lli->ctl_lo = 0;
        current_lli->ctl_hi = 0;

        // Default Configuration
        // TBD:
        dma_xfer_width(current_lli, 5, 5);
...
static void chan_xfer_start(struct dma_chan *chan, struct dma_lli *first){
    ...
    assert(!(dma_chan_is_enabled(chan)));
    ...
    write_chan_llp(chan, (uint64_t)first | lms);
...
static void chan_xfer_complete(struct dma_chan *chan, uint64_t chan_status)
{
    ...
    if (!completed_desc) {
        atomic_printf("Channel %d completed, but no active descriptor found!", chan->id);
        chan->chan_status = DWAXIDMAC_CH_IDLE;
        return;
    }
    ...
    if (chan_status & DWAXIDMAC_IRQ_ALL_ERR) {
        ...
        atomic_printf("DMA task on channel %d completed with error. Status: 0x%llx", chan->id, chan_status);
    }
    ...
    atomic_printf("DMA task on channel %d completed and resources freed.", chan->id);
}
...
static void dmac_intr_handler(){
    uint8_t dmac_id = 0;    // TBD
    uint64_t dmac_status, chan_status;
    int chan_id = -1;
    // int error_code = 0;
    ...
    dmac_status = READ_U32(dmac->base_addr + DMAC_COMMON_INTSTATUS);
    ...
    chan = &dmac->channels[chan_id];

    // active_desc = chan->active_desc;

    chan_status = READ_U64(chan->base_addr + CH_INTSTATUS);
    assert(chan_status);
...
static int dmac_intr_init() {
  printf("Initializing dmac interrupt ...\n");
...
int dma_transfer(enum xfer_direction direction, uint64_t src_addr, uint64_t dst_addr, uint32_t length, dma_callback call_back, void *user_data){
    int ret = 0;
